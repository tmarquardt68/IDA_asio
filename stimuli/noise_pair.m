function output = noise_pair(varargin)
%
% noise_pair.m produces a pair of binaural, bandpassed noise tokens with an
% optional pink spectrum.
%
% Parameters: 
%  duration: length of a component token in ms
%  cos_ramp: duration of the cosine squared ramp in ms
%  bandwidth: width of the bandpass filter in Hz.
%  freq: center of the bandpass filter in Hz.
%  ild: interaural level difference in dB
%  itd: interaural time difference in us
%  ipd: interaural phase difference in degrees
%  coherence: binaural coherence ([0 1]).
%  pink_spctrm: logical, 1=pink, 0=white
%  ioi: inter-onset interval between component tokens (ms)
%  deviant: 1 if deviant, 0 if standard
%  singleton: 0 for noise pair, 1 for initial noise only, 2 for second
%             noise only.
%
%
...
%
% Version history:
% 1.0  - derived from pink_noise v 1.1
%
% Part of Stimulus Toolbox by Bjorn Christianson
% Date: 11-04-13
vers = '1.0';
...
defaults.param(n).ToolTip = ...
	'If pink = 1, the level in dB has arbitrary reference';
...
defaults.param(n).ToolTip = 'Interaural time difference';
...
defaults.param(n).value = 0;
defaults.param(n).ToolTip = 'Interaural phase difference';
...
defaults.param(n).value = 0;
defaults.param(n).ToolTip = 'Interaural level difference';
...
defaults.param(n).ToolTip = 'Centre of bandpass filter';
...
defaults.param(n).ToolTip = 'Bandwidth of bandpass filter';
...
defaults.param(n).ToolTip = 'Duration of cosine squared ramp';
...
defaults.param(n).stepsize = 100;
defaults.param(n).value = 50;
defaults.param(n).ToolTip = 'Duration of single noise component';

n = n+1;
defaults.param(n).name = 'IOI [ms]';
defaults.param(n).min = 0;
defaults.param(n).max = 9999;
defaults.param(n).stepsize = 100;
defaults.param(n).value = 150;
defaults.param(n).ToolTip = 'Inter-onset interval of noise components';

n = n+1;
defaults.param(n).name = 'singleton';
defaults.param(n).min = 0;
defaults.param(n).max = 2;
defaults.param(n).stepsize = 1;
defaults.param(n).value = 0;
defaults.param(n).ToolTip = '0 for pair, 1 for first only, 2 for second only';
...
ioi = round(IOI/1000 * SAMPLE_RATE);
...
waves=cell(1,2);
for n=1:2
    if (n==1&&singleton~=2) || (n==2&&singleton~=1)
    ...
    waves{n} = wave(1:duration,:) .* (amplitude' * ...
        [cos(pi/2:(pi-pi/2)/(cos_ramp-1):pi).^2, ...
        ones(1,duration - 2*cos_ramp),cos(0:pi/2/(cos_ramp-1):pi/2).^2])';
    else
        waves{n}=zeros(duration,2);
    end
end

wave=zeros(duration+ioi,2);
wave(1:duration,:)=waves{1};
wave(ioi+(0:duration-1),:)=wave(ioi+(0:duration-1),:)+waves{2};
% IAC = corrcoef(wave);
% IAC = IAC(1,2),
% % Power: 
