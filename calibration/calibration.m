...
% ...\ida\calibration\calibration_data as a mat-file.
%
...
switch action
    case 'ini'
        if ~exist('calibration_data','dir')
            eval('!mkdir calibration_data')
        end

        h_calib = findobj('Tag','calibration_fig');
        if(isempty(h_calib))
            h_calib = calibration_fig;
        else
            figure(h_calib)
        end
    ...
    case 'fill'    % fill listbox with references
        ...
        str = getfield(what('calibration_data'),'mat');
        if (isempty(str))
            return;
        end
        files(length(str),30)=' ';
        ...
        path_str = ['calibration_data\',str{val}];
        try
    ...
    case 'retry'
        h_fig = findobj('Tag','calibration_fig');

        % save data of previous attempt as last trial
        if exist('calibration_data\00000__NONE__.mat','file')
            load ('calibration_data\00000__NONE__.mat'),
        elseif exist('calibration_data\00000_last_trial.mat','file')
            load ('calibration_data\00000_last_trial.mat')
        else
            tmp = zeros(1,l);
        ...
        save('calibration_data\00000_last_trial.mat',...
        ...
        set(h_fig,'Name','IDA Calibration in progress')
        drawnow
        ...
        snr_TT = 0.5*zeros(length(snr_TT),1);
        ...
        wave_in = data_acquisition([zeros(length(wave_HF),1), ...
            wave, zeros(length(wave_HF),1)],...
        ...
        min_freq = MIN_FREQ;
        save('calibration_data\00000__NONE__.mat',...
            'H_rec1','H_rec2','H_TT','H_mic','snr_rec1','snr_rec2', ...
            'snr_TT','mic_level_correction','max_level_rec1','comment',...
            'max_level_rec2','max_level_TT','min_freq','sample_rate',...
            'BK_amplitude_94dBspl'),

        set(h_fig, 'Name', 'IDA Calibration finished - now plotting')
        calibration('fill'),
        calibration('plot'),
        set(findobj(gcbf,'Tag','retry'),'String','Retry'),
        set(h_fig,'Name','IDA Calibration done. OK or RETRY?')
    ...
    case 'plot'
        ...
        load (['calibration_data\',reference])
        [m n] = size(H_rec1);
        if m == 1
        ...
        if exist('calibration_data\00000__NONE__.mat','file')
            load('_calibration_data\00000__NONE__')
        end
        ...
        axis([1 up_lim*MIN_FREQ -80 20]),grid on,
        ...
        axis([1 up_lim*MIN_FREQ -50 120]), grid,
    ...
    case 'ok'
        ...
        save('calibration_data\00000_last_calibration',...
            'H_rec1','H_rec2','H_TT','H_mic','snr_rec1','snr_rec2', ...
            'snr_TT','mic_level_correction','max_level_rec1','comment',...
            'max_level_rec2','max_level_TT','min_freq','sample_rate',...
            'BK_amplitude_94dBspl'),

        if ~exist('calibration_data\all','dir')
            eval('!mkdir calibration_data\all')
        end

        save([OAE_PATH,'calibration_data\all\cal',...
            num2str(now*10^9)],...
            'H_rec1','H_rec2','H_TT','H_mic','snr_rec1','snr_rec2',...
            'snr_TT','mic_level_correction','max_level_rec1','comment',...
            'max_level_rec2','max_level_TT','min_freq','sample_rate',...
    ...
    case 'save' % save calibration as special name to keep as reference
        ...
        str = ['calibration_data\', new_reference];
        if exist(str,'file')
            if ~strcmp(questdlg(strvcat('File: ', str, ...
                    ' already exists. Save anyway?'),...
                    'WARNING - IDA calibration:','No'),'Yes')
                return,
            end
        end
        try
    ...
    case 'delete_reference'
        ...
        if (strcmp(questdlg(strvcat('Are you sure you want delete : ',...
                item),'WARNING - Calibration:','No'),'Yes'))
            delete (['calibration_data\',item])
            set(h,'Value',1);
            calibration('fill'),
        end
    ...
    case 'help'
        msgbox (strvcat('ERROR calibration.m:',...
