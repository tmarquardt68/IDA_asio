...
disp('!!! Replace monitor_CAP_growth.m with latest monitor_combi.m !!!')
return
...
f1 = results.stimulus.parameters(3).values;
f2 = results.stimulus.parameters(4).values;
l1 = results.stimulus.parameters(5).values;
l2 = results.stimulus.parameters(6).values;
l_suppr = results.stimulus.parameters(7).values;
...
H_wave_suppr1 = fft(...
    ...
    + wave(52400 + latency+1 : fs/10 + 52400 + latency,:))/2);
...
plot([f1 f2 f_BT],[l2(prmSet)+l1 l2(prmSet) l_BT],'ko')
...
grid on, title(['CAP - L_B_T=' num2str(l_BT) 'dB, L2=' num2str(l2(prmSet)) 'dB @ ' num2str(f2) 'Hz'])
