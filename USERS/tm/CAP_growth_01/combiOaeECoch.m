...
% phase correction
phase_dev_1 = angle(CURRENT_EAR(f1/5+1,1));
phase_dev_2 = angle(CURRENT_EAR(f2/5+1,2));
phase_dev_BT = angle(CURRENT_EAR(f_BT/5+1,1));
Hdb_1 = 20*log10(abs(CURRENT_EAR(f1/5+1,1)'));
Hdb_2 = 20*log10(abs(CURRENT_EAR(f2/5+1,2)'));
Hdb_suppr1 = 20*log10(abs(CURRENT_EAR(f_suppr1/5+1,2)'));
Hdb_suppr2 = 20*log10(abs(CURRENT_EAR(f_suppr2/5+1,1)'));
Hdb_BT = 20*log10(abs(CURRENT_EAR(f_BT/5+1,1)'));

if max(l1 - Hdb_1) > CURRENT_EAR(1,5)
    msgbox(['The corrected Level L1 is ',...
        num2str(max(l1 - Hdb_1) - CURRENT_EAR(1,5)) ...
        ' dB higher than max_level!'],...
        'ERROR - DP_TT Measurement:','error');
    return,
end
if max(l_BT - Hdb_BT) > CURRENT_EAR(1,5)
    msgbox(['The corrected Level L_TT is ',...
        num2str(max(l_BT - Hdb_BT) - CURRENT_EAR(1,5)) ...
        ' dB higher than max_level!'],...
        'ERROR - DP_TT Measurement:','error');
    return,
end
if max(l1 + l_suppr - Hdb_suppr1) > CURRENT_EAR(2,5)
    msgbox(['The corrected Level L_suppr1 is ' ...
        num2str(max(l1 + l_suppr - Hdb_suppr1) - CURRENT_EAR(2,5)) ...
        ' dB higher than max_level!'],...
        'ERROR - DP_TT Measurement:','error');
    return,
end
if max(l2 + l_suppr - Hdb_suppr2) > CURRENT_EAR(1,5)
    msgbox(['The corrected Level L_suppr2 is ' ...
        num2str(max(l2 + l_suppr - Hdb_suppr2) - CURRENT_EAR(1,5)) ...
        ' dB higher than max_level!'],...
        'ERROR - DP_TT Measurement:','error');
    return,
end
if max(l2 - Hdb_2) > CURRENT_EAR(2,5)
    msgbox(['The corrected Level L2 is',...
        num2str(max(l2 - Hdb_2) - CURRENT_EAR(2,5)) ...
        ' dB higher than max_level!'],...
        'ERROR - DP_TT Measurement:','error');
    return,
end
...
ampl_suppr1 = 10^((l1+l_suppr-Hdb_suppr1-CURRENT_EAR(2,5))/20);
ampl_suppr2 = 10^((l2+l_suppr-Hdb_suppr2-CURRENT_EAR(1,5))/20);
...
duration_total = (6+1+1+32+1)*9/f_BTdivFs;
...
tone_suppr1 = ampl_suppr1*sin(linspace(0,f_suppr1/fs*duration_total*2*pi*(1-1/duration_total),duration_total));
tone_suppr2 = ampl_suppr2*sin(linspace(0,f_suppr2/fs*duration_total*2*pi*(1-1/duration_total),duration_total));
...
env_1 = [zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) ...
    ...
    ...
    zeros(1,2.5/f_BTdivFs) ramp_on duration ramp_off zeros(1,9/f_BTdivFs) ...
...
env_suppr1 = [zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) ...
    zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) ...
    zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) ...
    ...
    zeros(1,2/f_BTdivFs) ramp_on duration_2 ramp_off zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs)...
    zeros(1,round((32+1)*9/f_BTdivFs))];

env_suppr2 = [zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) ...
    zeros(1,2.5/f_BTdivFs) ramp_on duration ramp_off zeros(1,2.5/f_BTdivFs) ramp_on duration ramp_off ...
    zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) zeros(1,2.5/f_BTdivFs) zeros(1,6.5/f_BTdivFs) ...
    ...
    zeros(1,round((1+1+32+1)*9/f_BTdivFs))];

env_BT = [zeros(1,((6+1)*9)/f_BTdivFs) ...
